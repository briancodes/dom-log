...
// Don't run scrollIntoView if library is in an iframe
const _allowScrollIntoView = window.self === window.top;

let _logToConsole = false;
let _consoleElementId = 'app';
let _scrollIntoView = _allowScrollIntoView;
...
const sideBySide = (num = 2) => {
  ...
  const lastElements = Array.from(appElement.children || [])
    .slice(-num)
    .map(elem => {
      const width = '1%'; // items will expand to container size
...
/**
 * Scroll logged items into view
 *
 * @param {boolean} [value=true]
 */
const scrollIntoView = (value = true) => {
  _scrollIntoView = value && _allowScrollIntoView;
  return DomLog;
};
